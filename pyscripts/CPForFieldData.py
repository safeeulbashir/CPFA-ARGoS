...
import signal
# Example Call
# python pyscripts/CPForFieldData.py -w 60 -m 10 -l 5400 -d 1 -c 4 -detrending 2 CPFA_saves/RugosusFieldData/
# -w =Window Size
# -m sliding amount
# -l Experiment Length
# -d Distribution Type
...
if __name__ == "__main__":
    ...
    path = sys.argv[13]+"FoagingRecording/"
    ...
    for file in dirs:
        ...
        for d, t in zip(data['Dist'], data['Drop']):
            if t>=5400:
                t=5400
                collection_times[d][t]=5400
            elif t >= 0:
                t = t 
        ...
        print file
        pathForPheromone=file #Given Just to avoid Unnecessary Coding gap
        proc=subprocess.Popen(["Rscript", "pyscripts/CPRForFieldData.R",saveInto,str(change_points),str(slide_movement),pathForPheromone,str(pile),dirname,str(detrend)])
        #break
#print data
